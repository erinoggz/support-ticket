...
// middleware to give acces to AGENTS and ADMINS
export default (req: IRequest, res: IResponse, next: () => void) => {
  if (req.user.userType !== UserType.ADMIN && req.user.userType !== UserType.AGENT) return res.serverError(null, ResponseMessages.UNAUTHORIZED, StatusCodes.UNAUTHORIZED);
  next();
};
